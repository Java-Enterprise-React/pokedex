...
function Dashboard() {
    ...
    useEffect(() => {
        async function getAllPokemon(){
            console.log(process.env.REACT_APP_POKEMON_URL);
            let response = await fetch(`${process.env.REACT_APP_POKEMON_URL}?limit=151`);
    ...
    async function getPokemon(event : any){
        ...
        let response = await fetch(`${process.env.REACT_APP_POKEMON_URL}/${pokeName}`);
